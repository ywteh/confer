...
def main():
  cobi_data = open(p+'/confer-likes.csv').read()
  lines = re.split('\n', cobi_data)
  lines = map(lambda x: x.strip(), lines)
  lines = filter(lambda x: x.strip()!='', lines)
  tokens = [re.split(',', d) for d in lines]

  cobi_likes = {}
  for d in tokens:
    likes = re.split(';', d[3])
    likes = filter(lambda x: x.strip()!='' and x.strip!=';', likes)
    if len(likes) > 0:
      cobi_likes[d[0]] = set(likes)
  
  cobi_likes_count = [len(cobi_likes[k]) for k in cobi_likes]
  
  cobi_paper_likes = defaultdict(set)
  for id, likes in cobi_likes.iteritems():
    for l in likes:
      cobi_paper_likes[l].add(id)

  cobi_paper_likes_count = [len(cobi_paper_likes[k]) for k in cobi_paper_likes]

  cobi_affinity = defaultdict(dict)

  for p1 in cobi_paper_likes:
    for p2 in cobi_paper_likes:
      cobi_affinity[p1][p2] = len(cobi_paper_likes[p1].intersection(cobi_paper_likes[p2]))
